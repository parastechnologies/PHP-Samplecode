...
var initialElements  = function(canvas_info) {
    ...
    <div class="wrapper-memes-preview-operations">
             <button  class="trigger-element btn btn-sm btn-default" id="amm-brushes">
                  ...
                  <button style="display:none" class="responsive-button hide-responsive-button last btn btn-sm btn-default" id="amm-preview-responsive">
                             <span class="glyphicon glyphicon-eye-open"></span>             <span class="text_value">`+canvas_info.i18n.previewText+`</span>
            </button>      
                  <button href="#" style="display:none" class="trigger-element responsive-button hide-responsive-button first fileUpload btn btn-sm btn-default" 
    ...
    MemeGenerator = {
       ...
       get_images_class: function() {
        ...
        get_canvas_info: function() {
           // console.log(canvas_info, "canvas_info");
            return canvas_info;
        },
...
//get_texts
   ...
   document.getElementById("plus").addEventListener("click", function() {
    ...
    var img = canvas_info.img;
    var scale = 1.0;
    var scaleMultiplier = 0.8;
    var startDragOffset = {};
    var translatePos = {
        x: canvas_info.final_width / 2,
        y: canvas_info.final_height / 2
    };
    //canvas_info.ctx.translate(translatePos.x, translatePos.y);
      scale /= scaleMultiplier;
      //canvas_info.ctx.scale(scale, scale);

    //canvas_info.ctx.drawImage(img, 0, 0,img.width,img.height, 0, 0, canvas_info.final_width,canvas_info.final_height)
    MemeGeneratorAPI.canvas_reset(canvas_info.img,scale,canvas_info.width / 2,canvas_info.height / 2);
      }, false);

    }
    function initialize()
    {
         /*var c = document.getElementById("clothe-tshirt-maker");
              var ctx = c.getContext("2d");
              var img = document.getElementById("scream");
              ctx.drawImage(img, 5, 5);*/

      /*  c = container.find(selectors.ammCanvas);
        ctx = c.getContext("2d");
        var img = document.getElementById("scream");
        ...
        //ctx.drawImage(img, 5, 5);
         // var imgData = canvas_info.ctx.createImageData(1000, 100);
        //  container.find( ".wrapper_select_image_amm:first" ).click();            
          ...
          console.log(container.find( ".wrapper_select_image_amm:first" ).click(),"click")
    ...
    // When document has finished loading
     ...
     container.on('click', ".add_textbox", function(event) {
        ...
        $(".add_textbox").removeClass("add_textbox");    
           var text_info = {
                toUpperCase: true,
                toBoldCase:'bold',
                id: null,
            ...
            container.find('.amm-text-boxes').append(textarea);
             ...
             delete_box.on('click tap touchstart',function(){
                 ...
                 texts[text_info.id] = null;
                  ...
                  /*var translatePos = {
                    x: canvas_info.width / 2,
                    y: canvas_info.height / 2
                  };
            ...
            .resizable({
             ...
             if($(this).hasClass("opened")){
               ...
               $(this).removeClass("opened");
                ...
                texts[text_info.id].elementChanged = $(".moreOption");
                MemeGeneratorAPI.popupMoreOption(texts[text_info.id]);
                texts[text_info.id].elementChanged = null;
              }
...
container.on( "keyup", ".wrapper-text .amm-text", function(event){ 
            ...
            console.log(texts[id].text, "texttext");
...
container.on('click', '.moreOption', function(event){ 
     ...
     if($(this).hasClass("opened")){
      ...
      console.log(id,"id");
      texts[id].elementChanged = $(".moreOption");
      console.log(texts[id].elementChanged,"texts[id].elementChanged");
...
//      memeGeneratorData.memeGenerator = canvas_info.getImageData(0, 0, canvas_info.final_width, canvas_info.final_height);
    ...
    $.each(texts.keys, function(index, id) {
        ...
        console.log(container.find(selectors.wrapperHoverTitleMemeAmm).text()+'.png');
        ...
        /* start */
        $.ajax("https://maaracard.com/user/ajaxTest", {
            "data": "image=test",
            "type": "POST",
            //"timeout": 5000
        })
        .done(function (data, textStatus, jqXHR) {
            console.log(data, textStatus, jqXHR, "sdsadasd")
            // Process data, as received in data parameter
        })
        .fail(function (jqXHR, textStatus, errorThrown) {
            console.log(jqXHR, textStatus, errorThrown, "sdsadasd")
        ...
        /*$.ajax({
            url:"https://maaracard.com/user/ajaxTest",
            type:"POST",
            data:"image=dd",
 ...
 */
MemeGeneratorAPI.canvas_reset = function(img,scale,x,y) {
    console.log(img);
    var canvas_info = MemeGenerator.get_canvas_info();
  //console.log(canvas_info._maxWidth, img.width);
    ...
    container.find(selectors.ammCanvas+","+selectors.wrapperCanvas)
      ...
      //console.log(img1);                
      console.log(img, 0, 0,img.width,img.height, 0, 0, canvas_info.final_width,canvas_info.final_height)
    //canvas_info.ctx.drawImage(img1, 0, 0,img.width,img.height, 0, 0, canvas_info.final_width,canvas_info.final_height)
    canvas_info.ctx.translate(x, y);
    canvas_info.ctx.scale(scale, scale);
    canvas_info.ctx.drawImage(img, 0, 0,img.width,img.height, 0, 0, canvas_info.final_width,canvas_info.final_height)
...
//              console.log(["imagesid", id, image_object.img.src]);
        if(current_box_id == image_object.id)
             {   
                ...
                console.log("inis",image_object.img);
...
//        http://www.w3schools.com/tags/canvas_font.asp
        ...
        canvas_info.ctx.toBoldCase = MemeGeneratorAPI.buildBold(text_object);

        console.log(canvas_info.ctx,canvas_info.ctx.font,canvas_info.ctx.toBoldCase,"canvas_info.ctx");    
         // shadow
        ...
        canvas_info.ctx.textAlign =  text_object.textAlign;
         ...
         //canvas_info.ctx.bold = MemeGeneratorAPI.buildBold(text_object);
        
        canvas_info.ctx.toBoldCase = text_object.toBoldCase;

        if(text_object.toUpperCase) text = text.toUpperCase();

       //  if(text_object.toCurvedCase) text = text.bold();
...
MemeGeneratorAPI.buildBold = function(text_info)
{ 
    //console.log(text_info.toBoldCase,"text_info.toBoldCase");
    return text_info.fontWeight;
};
...
MemeGeneratorTheme.wrapper_text = function (text_info) { 
    ...
    var All_FontFamilies = ['Verdana','impact', 'Times New Roman', 'Arial', 'Courier New', 'serif', 'sans-serif','NanumGothic'];   
    ...
    var HTML_FontFamilies = '';
      var checked = '';
        ...
        var InputUpperCase = '<div class="wrapperInputUpperCase"><label>'+canvas_info.i18n.uperCaseText+': </label><input class="InputUpperCase EventChange" var="toUpperCase" value="'+text_info.toUpperCase+'" type="checkbox" '+((text_info.toUpperCase)?'checked="checked"':'')+'></div>';
        var InputCurvedCase = '<div class="wrapperInputCurved"><label>Curved: </label><input class="InputCurvedCase EventChange" var="toCurvedCase" value="'+text_info.toCurvedCase+'" type="checkbox" '+((text_info.toCurvedCase)?'checked="checked"':'')+'></div>';
        var InputBoldCase =  '<div class="wrapperInputBold"><label>Bold: </label><input class="InputBoldCase EventChange" var="toBoldCase" value="'+text_info.toBoldCase+'" type="checkbox"></div>'; 
